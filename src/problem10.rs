...
use crate::problem10::codec::{ParseError, PlainCommand};
...
pub async fn handle<T>(stream: T, _addr: SocketAddr, state: SharedState) -> anyhow::Result<()>
where
        ...
        match framed.next().await {
            ...
            Some(Err(e)) => {
                ...
                let message = match e {
                    ...
                    ParseError::MalformedCommand(PlainCommand::List) => "usage: LIST dir".into(),
                    ParseError::MalformedString => "strings must be UTF8 valid".into(),
                    ParseError::UnknownCommand(command) => format!("illegal method: {command}"),
